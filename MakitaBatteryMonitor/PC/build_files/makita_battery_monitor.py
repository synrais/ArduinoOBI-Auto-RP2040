...
CONNECT_WAIT = 1.0
VERSION      = "1.0.0"
DEVICE_HINTS = ["makita battery monitor", "rp2040", "pico", "waveshare", "usb serial device",
                "arduino nano", "ch340", "ch341", "usb2.0-serial", "usb-serial", "usb serial port"]
...
def wait_for_device() -> str:
    print(f"{CYAN}Searching for Makita Battery Monitor device...{RESET}")
    while True:
...
def print_header(port: str):
    ...
    print(f"{CYAN}{BOLD}  Makita Battery Monitor  v{VERSION}{RESET}")
...
def main():
    set_title("Makita Battery Monitor")
    print(f"\n{CYAN}{BOLD}  Makita Battery Monitor  v{VERSION}{RESET}\n")
