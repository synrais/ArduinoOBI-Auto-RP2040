...
static float calc_health_type56(const RawBasicInfo &raw) {
    int16_t f_ol = max((int16_t)raw.overload    - 29, (int16_t)0);
...
//  Temperature
// ─────────────────────────────────────────────
static float read_temperature(BatteryType t) {
    switch (t) {

        case BATT_TYPE_0: case BATT_TYPE_2: case BATT_TYPE_3: {
            static const uint8_t cmd[] = {0xD7, 0x0E, 0x00, 0x02};
            uint8_t rsp[3] = {0};
            if (cmd_cc(cmd, sizeof(cmd), rsp, 3) != BUS_OK) return TEMP_INVALID;
            if (rsp[2] != TYPE_PROBE_MAGIC) return TEMP_INVALID;
...
static void read_voltages_type023(float *v_pack, float cells[10],
                                   uint8_t cell_count) {
    static const uint8_t cmd[] = { 0xD7, 0x00, 0x00, 0x0C };
    uint8_t rsp[13] = {0};
    if (cmd_cc(cmd, sizeof(cmd), rsp, 13) != BUS_OK) return;
    if (rsp[12] != TYPE_PROBE_MAGIC) return;
...
static void read_voltages_type5(float *v_pack, float cells[10],
    ...
    for (uint8_t i = 0; i < n; i++) {
        ...
        uint8_t rsp[2] = {0};
        if (cmd_cc(cmd, 1, rsp, 2) != BUS_OK) continue;
