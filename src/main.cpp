...
static const uint8_t TYPE5_CMD_SESSION = 0x99;   // enters model-read session
static const uint8_t TYPE5_CMD_MODEL   = 0x31;   // queries model bytes
static const uint8_t TYPE5_CMD_TEMP    = 0x52;   // reads cell temperature

// ─── Type 6 command bytes ─────────────────────────────────────
static const uint8_t TYPE6_VOLT_READ   = 0xD4;   // voltage read prefix
...
static const uint8_t  UNLOCK_MAX_CYCLES     = 6;
static const uint8_t  LOCK_MAX_ATTEMPTS     = 3;

// ─── LED flash parameters ────────────────────────────────────
static const uint8_t  LED_FLASH_COUNT       = 3;
static const uint16_t LED_FLASH_ON_MS       = 200;
static const uint16_t LED_FLASH_OFF_MS      = 100;
...
static void led_flash(Colour c) {
    for (uint8_t i = 0; i < LED_FLASH_COUNT; i++) { led_set(c); delay(LED_FLASH_ON_MS); led_off(); delay(LED_FLASH_OFF_MS); }
    led_set(c);
}
...
// Prints the three main checksums on one line: "0-15=ok 16-31=ok 32-40=ok"
// Pass ok=true for each checksum that matches, false if bad.
// Caller prints the label (e.g. "Checksums      : ") before calling.
...
static bool read_model_type5(char out[8]) {
    ...
    g_ow.write(TYPE5_CMD_SESSION, 0);
    delay(10);
    g_ow.reset();
    delayMicroseconds(420);
    g_ow.write(TYPE5_CMD_MODEL, 0);
    delayMicroseconds(90);
    uint8_t b0 = g_ow.read();
    delayMicroseconds(90);
    uint8_t b1 = g_ow.read();
    delayMicroseconds(90);
    digitalWrite(PIN_ENABLE, LOW);
...
static float read_temperature_type6() {
    ...
    if (ok) { delayMicroseconds(BUS_BYTE_GAP_US); g_ow.write(CMD_TEMP6[0], 0);
...
static void type5_raw_cc(const uint8_t *data, uint8_t dlen, uint8_t *rsp, uint8_t rlen) {
    digitalWrite(PIN_ENABLE, HIGH);
    delay(420);              // F0513 chip requires ~420 ms power-up settle — not a typo
    g_ow.reset();
    delayMicroseconds(420);
    g_ow.write(0xCC, 0);
...
static void read_voltages_type5(float *vpack, float cells[10], uint8_t ncells, float *t_cell) {
    ...
    for (uint8_t i = 0; i < n; i++) {
        r[0] = r[1] = 0;
        type5_raw_cc(&CELL_CMDS[i], 1, r, 2);
        uint16_t mv = le16(r);
        if (mv > 0 && mv != 0xFFFF) { cells[i] = mv / 1000.0f; pack += cells[i]; }
    }
    *vpack = pack;

    r[0] = r[1] = 0;
    type5_raw_cc(&TYPE5_CMD_TEMP, 1, r, 2);
    uint16_t traw = le16(r);
    if (traw > 0 && traw != 0xFFFF) *t_cell = traw / 100.0f;
}
...
static void print_report(const BatteryInfo &info, float vpack,
    ...
    if (info.cell_count == 0) { Serial.println(F("(no cells reported)")); return; }
...
static bool write_corrected_frame(uint8_t data32[BASIC_INFO_LEN]) {
    ...
    for (uint8_t i = 0; i < 10 && vr == BASIC_INFO_NO_RESPONSE; i++) {
        delay(420);
        vr = read_basic_info(verify);
    }
...
// ─── Scan steps ───────────────────────────────────────────────
// Scan-mode variant: preserves PRE_TYPE0 result that lock_read_frame discards.
...
static void step_read_health(BatteryInfo &info, const uint8_t d[BASIC_INFO_LEN]) {
    ...
    if (info.type==BATT_TYPE_5 || info.type==BATT_TYPE_6) {
        ...
        info.health.charge_level   = (info.type==BATT_TYPE_0) ? read_charge_level() : 0;
    } else {
...
static void step_handle_lock(BatteryInfo &info, uint8_t d[BASIC_INFO_LEN]) {
    ...
    for (uint8_t attempt = 1; attempt <= UNLOCK_MAX_CYCLES && !unlocked; attempt++) {
        ...
        if (attempt % 2 == 1) {
            ...
            parse_basic_info(d, info.raw); derive_battery_info(d, info);
    ...
    if (unlocked) {
        ...
        Serial.println(F("Battery still locked."));
        led_flash(COL_RED);
    }
}
...
static bool checksums_bad(const uint8_t *buf) {
    bool cs0_bad = (checksum_calc(buf, 0,  15) != nybble_get(buf, 41));
    bool cs1_bad = (checksum_calc(buf, 16, 31) != nybble_get(buf, 42));
    bool cs2_bad = (checksum_calc(buf, 32, 40) != nybble_get(buf, 43));
    return cs0_bad || cs1_bad || cs2_bad;
}

// Shared retry loop used by both scan and lock paths.
// Returns true only on BASIC_INFO_OK; leaves frame contents as read on last attempt.
static bool lock_read_frame(uint8_t frame[BASIC_INFO_LEN], uint8_t retries = SCAN_MAX_RETRIES) {
    for (uint8_t i = 0; i < retries; i++) {
        if (read_basic_info(frame) == BASIC_INFO_OK) return true;
        delay(SCAN_RETRY_DELAY_MS);
    }
    return false;
}
...
static bool lock_apply(uint8_t frame[BASIC_INFO_LEN]) {
    ...
    for (uint8_t attempt = 1; attempt <= LOCK_MAX_ATTEMPTS; attempt++) {
        ...
        // Corrupt checksums by adding attempt offset — forces battery into locked state
        nybble_set(v, 40, (checksum_calc(v, 32, 40) + attempt) & 0xF);
        nybble_set(v, 41, (checksum_calc(v,  0, 15) + attempt) & 0xF);
        nybble_set(v, 42, (checksum_calc(v, 16, 31) + attempt) & 0xF);
        nybble_set(v, 43,  checksum_calc(v, 32, 40));

        led_purple();
        if (enter_testmode() != BUS_OK) { Serial.println(F("  [Lock] No presence entering testmode.")); return false; }
        if (!charger_write_frame(v))    { exit_testmode(); power_cycle_bus(); return false; }
        exit_testmode();
        power_cycle_bus();

        if (!lock_read_frame(v, 10)) { Serial.println(F("  [Lock] No response.")); return false; }

        bool cs0 = (checksum_calc(v, 0,  15) != nybble_get(v, 41));
        bool cs1 = (checksum_calc(v, 16, 31) != nybble_get(v, 42));
        bool cs2 = (checksum_calc(v, 32, 40) != nybble_get(v, 43));
        Serial.print(F("  [Lock] Checksums : ")); print_checksums(!cs0, !cs1, !cs2);
        if (cs0 || cs1 || cs2) return true;
...
static bool run_lock() {
    ...
    if (!lock_read_frame(frame)) {
        // Distinguish HC08 (all-0xFF frame) from genuine no-response
        uint8_t andv = 0xFF;
        for (uint8_t i = 0; i < BASIC_INFO_LEN; i++) andv &= frame[i];
        if (andv == 0xFF) { Serial.println(F("  [Lock] Pre-type-0 HC08 — unsupported.")); led_yellow(); }
    ...
    print_sep();
    if (ok) led_flash(COL_GREEN); else led_flash(COL_RED);
    bus_disable();
    return ok;
}
...
static bool run_scan() {
    ...
    if (info.type==BATT_TYPE_6)
        info.temperature_c = read_temperature_type6();
        ...
        if (raw == g_last_lock_mode) {
            ...
            if (g_state==IDLE || g_state==WAIT_BATTERY || g_state==UNSUPPORTED) {
                ...
                if (lock_mode) {
                    ...
                    Serial.println(F("  Bridge GPIO4-GPIO5 detected."));
                    Serial.println(F("  Insert battery to lock."));
                } else {
